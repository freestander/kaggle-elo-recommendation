...
"""
Preprocessing pipeline for ELO data into trainable format
...
def historical_transactions(num_rows=None):
    ...
    aggs['month_diff'] = ['max','min','mean','var','skew','std']
...
def additional_features(df):
    ...
    ## Rob adding this for average from
    # df['hist_avg_buy'] = (df['hist_purchase_date_mean'] - df['first_active_month']).dt.days
    # df['new_avg_buy'] = (df['new_purchase_date_mean'] - df['first_active_month']).dt.days
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified = False, debug= False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
        ...
        nowtime = dt.datetime.now().strftime("%Y-%m-%d-%H%M%S")
        score_text = '{} Fold {} RMSE : {:.6f}'.format(nowtime, n_fold + 1, rmse(valid_y, oof_preds[valid_idx]))
        append_log_file(score_text)
